...
<!-- 	    댓글 작성부분 -->
		...
		<input type ="hidden" name="search" value=<%=search %>>
	   	<div class="reply_write">
	   		<input class="reply_input" readonly>
			<input class="btn_style_2" value="&nbsp;&nbsp;&nbsp;댓글등록">
		</div>
		</form>
		<br>
<!-- 		댓글 리스트 출력부분 -->
...
<!-- 			댓글 수정 코딩 -->
			<form action="comment_modify_update.jsp?uid=<%=uid %>&code=<%=code %>&c_uid=<%=c_uid %>&page_now=<%=page_now %>" method="post">
...
<!-- 				무슨 값을 비교해서 input을 지정할수있을까나.... -->
				<% if(c_uid.equals(cc_uid)){ %>
					<input class="reply_view2" id="<%=cc_uid %>" name="comment_modify" value="<%=c_comment %>"  onsubmit="">
					<script>
...
<!-- 					/////////////////////////////// -->
				...
				<%}else{ %>
				<div class="reply_view2"><%=c_comment %>*** c_uid:<%=cc_uid %></div>
				<%} %>
			<%}else{ %>
				<div class="reply_view2"><%=c_comment %>*** c_uid:<%=cc_uid %></div>
			<%} %>
