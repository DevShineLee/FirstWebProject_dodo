...
if(request.getParameter("field") != null && request.getParameter("field") != ""){ //검색이 있을 경우 (공지는 카운트 안함.)
	sql_count = "select count(*) as total_count from "+code+" where gongji != '1' and "+field+" like '%"+search+"%'";
}else{
	sql_count = "select count(*) as total_count from "+code+" where gongji != '1'";
}
...
int num_per_page = 4; //한 페이지당 출력할 게시물 수
...
<div class="board_list_wrap">
    <div class="board_list">
		...
		while(rs_gongji.next()){
			...
			today_db = rs_gongji.getString("signdate");		
			String file1 = rs_gongji.getString("file1");
			String file1_s = rs_gongji.getString("file1_s");
...
<!--			썸네일 출력 구문	 -->
		        <div class="thum">
		        	<a href="view.jsp?code=<%=code%>&uid=<%=uid%>&page_now=<%=page_now%>&field=<%=field%>&search=<%=search%>">
		        		<img class="img" src="/upload/<%=file1_s%>" onerror="this.src='img/star1.png'">
		        	</a>
		        </div>
	            <div class="title">
	            	<%
					...
					<%}%>
					<%
					String sql_c_count = "select count(*) as comment_count from comment_g where tb_uid="+uid+"";
		        	...
		        	<a href="view.jsp?code=<%=code%>&uid=<%=uid%>&page_now=<%=page_now%>&field=<%=field%>&search=<%=search%>">
		        		<%if(gongji.equals("3")) {%>
		        			<%if((session_level != null && session_level.equals("10")) || (session_id != null && session_id.equals("id"))){ %>
		        				<img class="img" src="/upload/<%=file1_s%>" onerror="this.src='img/star1.png'">
		        			<%}else{ %>
		        				<img class="img" src="img/secrete.png">
		        			<%} %>
		        		<%}else{ %>
	        				<img class="img" src="/upload/<%=file1_s%>" onerror="this.src='img/star1.png'">
	        			<%} %>
...
%>			
    ...
    <div class="bt_wrap">
 		<%if(session_level != null && !session_level.equals("")){
		%>
         ...
         <%}else{%>
         	...
         	<a href="/member/login.jsp?code=<%=code %>" class="off">로그인하기</a>
        <%
         }
     	%>
    </div>
